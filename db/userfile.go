...
func QueryUserFileMeta(username string, filesha1 string) (*UserFile, error) {
	stmt, err := mydb.DBConn().Prepare(
		"select file_sha1,file_name,file_size,upload_at,last_update from tbl_user_file where user_name=? filesha1=?")
	if err != nil {
		return nil, err
	}
	defer stmt.Close()
	rows, err := stmt.Query(username, filesha1)
	if err != nil {
		return nil, err
	}
	ufile := UserFile{}
